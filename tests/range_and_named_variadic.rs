...
fn range_as_call_argument_is_rejected() {
    let src = "package m\nstruct Box {\nv: Object\npublic func new(v: Object): Self { return Self { v: v } }\n}\nstruct Main {\npublic func run(args: String...): Long {\nlet b: Box = Box.new(1..5)\nreturn 0\n}\n}\n";
    let err = compile_err(src);
    assert!(err.contains("C147"), "got: {err}");
    assert!(!err.contains("V005"), "got: {err}");
}
...
fn named_variadic_argument_supplies_the_list() {
    let code = run(&format!(
        "{SUM_SRC}\npublic func run(args: String...): Long {{ return Main.sum(xs: [1, 2, 3]) }}\n}}\n"
    ));
    assert_eq!(code, 6);
}
...
fn named_variadic_empty_list_matches_spread_semantics() {
    ...
    let named = format!(
        "{SUM_SRC}\npublic func run(args: String...): Long {{ return Main.sum(xs: []) }}\n}}\n"
    );
    let err = compile_err(&named);
    assert!(err.contains("C181"), "got: {err}");
    let spread = format!(
        "{SUM_SRC}\npublic func run(args: String...): Long {{ return Main.sum(...[]) }}\n}}\n"
    );
...
fn positional_and_spread_variadic_baselines_unchanged() {
    let pos = run(&format!(
        "{SUM_SRC}\npublic func run(args: String...): Long {{ return Main.sum(1, 2, 3) }}\n}}\n"
    ));
    assert_eq!(pos, 6);
...
fn named_variadic_conflicting_with_positional_tail_is_rejected() {
    let src = format!(
        "{SUM_SRC}\npublic func run(args: String...): Long {{ return Main.sum(1, xs: [2]) }}\n}}\n"
    );
    let err = compile_err(&src);
...
fn named_variadic_non_list_value_is_rejected() {
    let src = format!(
        "{SUM_SRC}\npublic func run(args: String...): Long {{ return Main.sum(xs: 5) }}\n}}\n"
    );
    let err = compile_err(&src);
...
fn named_variadic_element_type_mismatch_is_rejected() {
    let src = format!(
        "{SUM_SRC}\npublic func run(args: String...): Long {{ return Main.sum(xs: [\"a\"]) }}\n}}\n"
    );
    let err = compile_err(&src);
    assert!(err.contains("C181"), "got: {err}");
}
...
fn named_arguments_for_regular_parameters_still_work() {
    let code = run("package m\nstruct Main {\n\
         public func add(a: Long, b: Long): Long { return a + b }\n\
         public func run(args: String...): Long { return Main.add(b: 2, a: 1) }\n\
         }\n");
    assert_eq!(code, 3);
}
