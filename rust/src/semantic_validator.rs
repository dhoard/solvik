...
use crate::semantic_types::{assignable, source_type_name, substitute, TypeParam, TypeRef};
...
impl<'a> Validator<'a> {
    ...
    fn validate_struct_decl(&mut self, structure: &StructDecl) -> Result<(), ValidationError> {
        ...
        for method in &structure.methods {
            if methods.insert(method.name.clone(), ()).is_some() { return Err(ValidationError::new("C091", format!("duplicate method '{}'", method.name))); }
            self.validate_function(method, Some(structure))?;
        }
        Ok(())
    }
    ...
    fn check_type(&self, typ: &TypeRef, params: &Vec<String>) -> Result<(), ValidationError> {
        ...
        if params.iter().any(|param| param == &typ.name) || self.traits.contains_key(&typ.name) || ["any", "exception", "regex", "bool", "byte", "int", "float", "char", "string", "thread", "mutex", "semaphore", "process", "instream", "outstream", "threaddef", "processdef"].contains(&typ.name.as_str()) { return Ok(()); }
        ...
        for (pattern, body) in cases {
            if let Some(pattern) = pattern {
                ...
                match pattern {
                    ...
                    _ => { if let Some(pattern_name) = expression_name(pattern) { if self.enums.contains_key(pattern_name.rsplit('.').next().unwrap_or(&pattern_name)) { return Err(ValidationError::new("C094", "switch case enum does not match switch enum")); } } let case_type = self.infer(pattern)?; if case_type.name == "null" && !switch_type.nullable { return Err(ValidationError::new("C094", "null case cannot match a non-nullable switch")); } let numeric = |name: &str| name == "byte" || name == "int" || name == "float"; // Cases may use any numeric type against any numeric switch value
                    // (wider/narrower matching at runtime); otherwise the
                    // case type must match the switch value type.
                    let overlaps = case_type.name == "unknown" || switch_type.name == "unknown" || case_type.name == "any" || switch_type.name == "any" || (case_type.name == "null" && switch_type.nullable) || case_type.name == switch_type.name || (numeric(&case_type.name) && numeric(&switch_type.name)); if !overlaps { return Err(ValidationError::new("C094", "switch case type does not match switch value")); } }
                }
    ...
    fn infer(&mut self, expression: &Expr) -> Result<TypeRef, ValidationError> {
        ...
        Ok(match expression {
            ...
            Expr::Call { callee, args, type_args } => { let explicit_args = match callee.as_ref() { Expr::Name { type_args, .. } | Expr::Member { type_args, .. } => type_args, _ => type_args }; if let Expr::Name { name, .. } = callee.as_ref() { if let Some(function) = self.functions.get(name).cloned() { if !function.type_params.is_empty() && !explicit_args.is_empty() && explicit_args.len() != function.type_params.len() { return Err(ValidationError::new("C096", "generic type argument count mismatch")); } if let Some(param) = function.params.last() { if param.variadic { for argument in args { let actual = self.infer(&argument.expr)?; if !compatible(&actual, &param.typ) { return Err(ValidationError::new("C101", "variadic argument type mismatch")); } } return Ok(function.return_type); } } for (argument, param) in args.iter().zip(&function.params) { let actual = self.infer(&argument.expr)?; if !function.type_params.is_empty() && !explicit_args.is_empty() { if let Some(index) = function.type_params.iter().position(|type_param| type_param.name == param.typ.name) { if index < explicit_args.len() && !compatible(&actual, &explicit_args[index]) { return Err(ValidationError::new("C101", "explicit type argument does not match argument")); } } } if let Some(type_param) = function.type_params.iter().find(|type_param| type_param.name == param.typ.name) { if !self.satisfies(&actual, type_param) { return Err(ValidationError::new("C095", format!("type {} does not satisfy generic constraint", actual))); } } } } } else if let Expr::Member { object, name, .. } = callee.as_ref() { if let Some(obj_name) = expression_name(object) { let local = obj_name.rsplit('.').next().unwrap_or(&obj_name); let shadowed = !obj_name.contains('.') && self.lookup(&obj_name).is_some(); if !shadowed { if let Some(structure) = self.structs.get(&obj_name).or_else(|| self.structs.get(local)).cloned() { match structure.methods.iter().find(|method| method.name == *name) { None => return Err(ValidationError::new("C126", format!("type '{}' has no associated function '{}'", obj_name, name))), Some(method) => { if !method.public && obj_name.contains('.') { return Err(ValidationError::new("C120", format!("associated function '{}' of '{}' is private", name, obj_name))); } let explicit: Vec<TypeRef> = match &**object { Expr::Name { type_args, .. } | Expr::Member { type_args, .. } => type_args.clone(), _ => Vec::new() }; let mut bindings: Vec<(String, TypeRef)> = Vec::new(); if !explicit.is_empty() { if explicit.len() != structure.type_params.len() { return Err(ValidationError::new("C096", format!("type '{}' takes {} type argument(s), found {}", obj_name, structure.type_params.len(), explicit.len()))); } bindings = structure.type_params.iter().zip(explicit.iter()).map(|(p, a)| (p.name.clone(), a.clone())).collect(); } let variadic = method.params.last().is_some_and(|p| p.variadic); let fixed = method.params.len() - usize::from(variadic); let type_param_names: Vec<String> = structure.type_params.iter().map(|p| p.name.clone()).collect(); if !variadic && args.len() != method.params.len() { return Err(ValidationError::new("C101", format!("{}.{} expects {} argument(s), found {}", obj_name, name, method.params.len(), args.len()))); } for (index, (argument, param)) in args.iter().zip(&method.params).enumerate() { let actual = self.infer(&argument.expr)?; let expected = if bindings.is_empty() { param.typ.clone() } else { substitute(&param.typ, &bindings) }; if !type_param_names.iter().any(|n| n == &expected.name) && !compatible(&actual, &expected) { return Err(ValidationError::new("C101", format!("argument {} of '{}.{}': expected {} but got {}", index + 1, obj_name, name, expected, actual))); } } if let Some(param) = method.params.last() { if param.variadic { let expected = if bindings.is_empty() { param.typ.clone() } else { substitute(&param.typ, &bindings) }; for argument in args.iter().skip(fixed) { let actual = self.infer(&argument.expr)?; if !type_param_names.iter().any(|n| n == &expected.name) && !compatible(&actual, &expected) { return Err(ValidationError::new("C101", format!("argument of '{}.{}': expected {} but got {}", obj_name, name, expected, actual))); } } } } let mut result = if bindings.is_empty() { method.return_type.clone() } else { substitute(&method.return_type, &bindings) }; if obj_name.contains('.') && result.name.rsplit('.').next().unwrap_or(&result.name) == local && structure.name != local { result.name = structure.name.clone(); } return Ok(result); } } } } } if let Some(enum_name) = expression_name(object) { if let Some(enumeration) = self.enums.get(enum_name.rsplit('.').next().unwrap_or(&enum_name)).cloned() { if let Some(member) = enumeration.members.iter().find(|member| member.name == *name) { if member.payload.len() != args.len() { return Err(ValidationError::new("C101", "enum construction payload count mismatch")); } for (argument, expected) in args.iter().zip(&member.payload) { let actual = self.infer(&argument.expr)?; if !compatible(&actual, expected) { return Err(ValidationError::new("C101", "enum construction payload type mismatch")); } } return Ok(TypeRef::named(&enumeration.name)); } } } } let typ = self.infer(callee)?; if typ.name == "func" && !typ.args.is_empty() { let params = &typ.args[..typ.args.len()-1]; if args.len() != params.len() { return Err(ValidationError::new("C101", "function argument count mismatch")); } for (argument, expected) in args.iter().zip(params) { let actual = self.infer(&argument.expr)?; if !compatible(&actual, expected) { return Err(ValidationError::new("C101", "function argument type mismatch")); } } } else if typ.name != "unknown" { return Err(ValidationError::new("C102", "value is not callable")); } typ.args.last().cloned().unwrap_or_else(unknown) }
            Expr::Member { object, name, .. } => { let typ = self.infer(object)?; if ["string", "list", "map", "stack", "thread", "mutex", "semaphore", "process", "instream", "outstream"].contains(&typ.name.as_str()) { unknown() } else if let Some(structure) = self.structs.get(&typ.name) { if let Some(field) = structure.fields.iter().find(|field| field.name == *name) { field.typ.clone() } else if let Some(method) = structure.methods.iter().find(|method| method.name == *name) { TypeRef::generic("func", method.params.iter().map(|param| param.typ.clone()).chain(std::iter::once(method.return_type.clone())).collect()) } else { unknown() } } else { unknown() } }
