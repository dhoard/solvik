...
usage() {
    ...
    echo "  clean            Remove dist/go/ directory"
...
case "${1}" in
    ...
    all)
        ...
        if command -v goreleaser &>/dev/null; then
            ...
            header "All-architecture build complete. Artifacts in ./dist/go/"
        else
    ...
    release-snapshot)
        ...
        goreleaser release --snapshot --clean
        header "Snapshot complete. Artifacts in ./dist/go/"
        ;;
    release)
